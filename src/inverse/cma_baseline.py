...
class CMABaseline:
    ...
    def run_gnn(self, samples, eval_labels):
        ...
        for split_idx in range(num_splits):
            ...
            for i in range(start_idx, end_idx):
                ...
                if self.config.get('data_set_type') == 'pixel_data_with_reflectors':
                    reflector_matrix = smooth_binarize(samples[i, 1], 300, 300) if samples.shape[1] > 1 else torch.zeros_like(samples[i, 0])
    ...
    def optimize_sample_with_cma(self, matrix, eval_label):
        def objective(x, target, grid_size=16):
            # x is a flattened tensor. We need to shape it to (1, 1, 16, 16) or (1, 2, 16, 16) for reflectors
            channels = 2 if self.config.get('data_set_type') == 'pixel_data_with_reflectors' else 1
            x_tensor = torch.tensor(x, dtype=torch.float32).reshape(-1, channels, grid_size, grid_size)
            pred_list, gt_list, losses, antenna_list, valid_indices = self.run_gnn(x_tensor, target)
            if not losses:
                return float('inf')
            loss = losses[0]
            return loss

        es = cma.CMAEvolutionStrategy(matrix.flatten().cpu().numpy(), 0.2)
        es.optimize(objective, args=(eval_label,), iterations=100)

        best_x = es.result.xbest
        channels = 2 if self.config.get('data_set_type') == 'pixel_data_with_reflectors' else 1
        best_matrix = torch.tensor(best_x, dtype=torch.float32).reshape(-1, channels, 16, 16).to(self.device)
        pred_list, gt_list, losses, antenna_list, valid_indices = self.run_gnn(best_matrix, eval_label)
    ...
    def test(self, eval_loader):
        ...
        for idx, batch in tqdm(enumerate(eval_loader), total=len(eval_loader), desc="Testing CMA baseline"):
            with torch.no_grad():
                gt_ant_matrix, eval_labels, class_labels, raw_idx = self.get_batch_data(batch)

                # We start with a random starting point
                channels = 2 if self.config.get('data_set_type') == 'pixel_data_with_reflectors' else 1
                start_matrix = torch.rand((1, channels, 16, 16)).to(self.device)

                best_matrix, min_loss, min_graph, min_antenna, min_farfield, gt_farfield = self.optimize_sample_with_cma(start_matrix, eval_labels)
...
if __name__ == "__main__":
    ...
    arg_parser = ArgParser(config_file=os.path.join(_REPO_ROOT, 'configs', 'inverse', 'cma_baseline.yaml'))
    ...
    #   data_root, split_dir, surrogate_checkpoint, output_dir
    ...
    eval_loader = DataLoader(test_dataset, batch_size=1, follow_batch=['pos_surface_current'], shuffle=False)
    ...
    baseline = CMABaseline(config, simulation_model)
