...
class GCN(nn.Module):
    """
    5-layer GCN for node classification.
    Layers: [in -> h -> h -> h -> h -> out]
    Includes BatchNorm, ReLU, Dropout, and lightweight residual connections.
    """
    def __init__(self, in_channels, hidden_channels, out_channels, dropout=0.5):
        super().__init__()
        self.convs = nn.ModuleList([
            GCNConv(in_channels, hidden_channels//4, add_self_loops=True, normalize=True),
            GCNConv(hidden_channels//4, hidden_channels//2, add_self_loops=True, normalize=True),
            GCNConv(hidden_channels//2, hidden_channels, add_self_loops=True, normalize=True),
        ...
        # For residuals: project input of each block to match dims when needed
    ...
    def reset_parameters(self):
        ...
        for bn in self.bns:
            bn.reset_parameters()
        for rp in self.res_proj:
            if hasattr(rp, "reset_parameters"):
                rp.reset_parameters()

    def forward(self, x, edge_index, batch, edge_weight=None):
        # 4 hidden GCN blocks with BN/ReLU/Dropout (+ residual)
        for i in range(4):
            h_in = x
            ...
            # residual (identity here since dims match hidden_channels)
            x = x + self.res_proj[i](h_in) if h_in.shape == x.shape else x

        # final logits (no BN/ReLU)
        x = self.convs[4](x, edge_index, edge_weight=edge_weight)
...
class GCNWrapper(nn.Module):
    def __init__(self, config, output_channels=40):
        ...
        self.setup_config()
        dropout = 0
        hidden_channels = self.config.get('hidden_channels', 256)
        current_dim = 6
        self.gcnn = GCN(in_channels=17, hidden_channels=hidden_channels, out_channels=output_channels).to(torch.device('cuda'))
        ...
        if self.config.get('trained_model_path_for_DGCNN'):
            ...
            self.gcnn.load_state_dict(torch.load(self.config.get('trained_model_path_for_DGCNN')))
            print('model loaded')
    ...
    def forward(self, data, radiation_image_shape=None, is_training=True):
        ...
        x = self.gcnn(x, edge_index, data.batch)
        # graph readout
