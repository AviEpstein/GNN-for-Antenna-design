...
def create_antenna_FMNIST_dataset(dataloader, config, display=False, second_dataloader=None):

    output_dir = config.get('output_dir', 'outputs/antenna_FMNIST_CIFAR/')
    all_images = []
    all_labels = []
    skipped_idxs = []
    ...
    for idx, (imgs, labels) in tqdm(enumerate(dataloader), total=len(dataloader)):
        try:
            ...
            ########### Create reflector from second dataloader ############
            ##############################################################
            # get a random sample from the second dataloader to create the reflector
            rand_idx = random.randint(0, len(second_dataloader.dataset) - 1)
            rand_img, _ = second_dataloader.dataset[rand_idx]
            reflector_matrix = rand_img[0] if rand_img.ndim == 3 else rand_img

            # here we create the reflector from the random image 
            ...
            reflector_distance = config.get('reflector_distance', 3) #3
            # move reflector up in z direction
            reflector.pos[:,2] = reflector.pos[:,2] + reflector_distance
            # Scale reflector in x/y
            scale = config.get('reflector_scale', 1.5)
            reflector.pos[:, :2] = reflector.pos[:, :2] * scale
            # Center reflector at origin in x/y
            # center = reflector.pos[:, :2].mean(dim=0, keepdim=True)
            # reflector.pos[:, :2] = reflector.pos[:, :2] - center           
            
            # now lets add the reflector to the antenna graph dict
            antenna_graph_dict['PEC_Reflector'] = reflector
            graph = merge_and_connect_graphs_from_dict(antenna_graph_dict, 'sphere', k=config.get('k_sphere_neighbores', 2), add_sphere=False)
            ...
            paramter_dict['reflector_matrix'] = reflector_matrix 
            paramter_dict['reflector_distance'] = reflector_distance
            paramter_dict['reflector_scale'] = scale
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="outputs/antenna_FMNIST_CIFAR/", help="Where to write the generated dataset")
    ...
    cifar10_second_ds = get_dataset(args.dataset, root, train_flag, Path(args.imagenet_dir) if args.imagenet_dir else None)
    second_dl = DataLoader(cifar10_second_ds, batch_size=1, shuffle=True, num_workers=args.num_workers)

    config = args_to_dict(args)
    
    create_antenna_FMNIST_dataset(dl, config, second_dataloader=second_dl)
